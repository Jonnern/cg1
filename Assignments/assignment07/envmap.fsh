...
void main() {
    ...
    if (uUseCubeMapping) {
        ...
        // Implement sphere mapping here by computing vec2 sphereCoord (Part a)
        vec2 sphereCoord = vec2(0, 0);
        vec3 texVec = normalize(vec3(reflectionVector) + vec3(0.0, 0.0, 1.0));
        sphereCoord = vec2((texVec.x + 1.0) / 2.0, (texVec.y + 1.0) / 2.0);
