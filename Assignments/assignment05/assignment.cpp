...
float evaluateF(const glm::vec2& p1, const glm::vec2& p2, const glm::vec2& p)
{
    ...
    // ====================================================================
    glm::vec2 line = p1-p2;
    glm::vec2 n(-line.y, line.x);
    return glm::dot(n, p) - glm::dot(n, p1);
    // ====================================================================
    // End Exercise code
    // ====================================================================
}
...
void drawTriangle( const glm::vec4& p0_in, const glm::vec4& p1_in, const glm::vec4& p2_in, const glm::vec3& normal) {
    ...
    // Add your code here:
    // ====================================================================
    minX = max(0, (int)min(min(v0.x, v1.x), v2.x));
    minY = max(0, (int)min(min(v0.y, v1.y), v2.y));
    maxX = min(g_windowWidth, (int)max(max(v0.x, v1.x), v2.x));
    maxY = min(g_windowHeight, (int)max(max(v0.y, v1.y), v2.y));
    // ====================================================================
    // End Exercise code
    // ====================================================================
    ...
    // Add your code here:
    // ====================================================================
    diffuse = glm::dot(normal, glm::vec3(0,0,1));
    if(diffuse < 0)
        return;
    ...
    for (p[1] = minY; p[1] <= maxY; p[1]++) {
        for (p[0] = minX; p[0] <= maxX; p[0]++) {
            ...
            if(evaluateF(v0, v1, p) < 0.0 && evaluateF(v1, v2, p) < 0.0 && evaluateF(v2, v0, p) < 0.0)
            {
...
void drawScene(int _scene, float _runTime) {
    ...
    if ( (_scene == 2) || (_scene == 3) ) {
        ...
        // Add your code here:
        // ====================================================================
        glm::mat4 rot(glm::transpose(glm::mat4(cos(_runTime), 0, -sin(_runTime), 0,
                                               0,             1, 0,              0,
                                               sin(_runTime), 0, cos(_runTime),  0,
                                               0,             0, 0,              1)));

        g_ModelViewMatrix = g_ModelViewMatrix*rot;
        // ====================================================================
        // End Exercise code
        // ====================================================================
